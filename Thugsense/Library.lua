...
local Library do
    ...
    Library.Round = function(self, Number, Float)
        -- synca mobile patch: Decimals=0 used to give 1/0=inf -> NaN values.
        -- Treat nil/0 decimals as 1 so sliders never display NaN.
    ...
    Library.Sections.Slider = function(self, Data)
        ...
        -- synca mobile patch: on touch devices MouseButton1Down is remapped to
        -- TouchTap (fires on release, no drag). Start sliding on Touch begin so
        -- mobile users can drag sliders with their finger.
        Items["RealSlider"]:Connect("InputBegan", function(Input)
            if Input.UserInputType == Enum.UserInputType.Touch then
                Slider.Sliding = true

                local TouchPos = Vector2New(Input.Position.X, Input.Position.Y)
        ...
        Library:Connect(UserInputService.InputChanged, function(Input)
            -- synca mobile patch: also track Touch drags, not just MouseMovement.
            if (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) and Slider.Sliding then
                local MousePos
                if Input.UserInputType == Enum.UserInputType.Touch then
                    MousePos = Vector2New(Input.Position.X, Input.Position.Y)
                else
